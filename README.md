...
**新闻选股助手** 是一个 Claude Code 技能（Skill），从新闻和热点中自动识别相关 A 股标的，通过**通达信本地数据全市场扫描**和量化 Tier 评分引擎产出结构化选股结果，并**生成**深色交易台风格的 HTML 日报。

### 五大核心能力

| 能力 | 说明 |
|------|------|
| **通达信全市场扫描** | 读取通达信本地 vipdoc 数据，5537 只 A 股全量扫描仅需 0.6 秒，无需网络 API |
| **8 大投资流派匹配** | 价值/成长/短线/技术/量化/事件驱动/宏观/投机，流派感知 Tier 引擎 + 搜索关键词轮换池 |
| **新闻驱动选股** | 从政策、业绩、并购、技术突破等 9 类利好新闻中自动识别 A 股候选标的 |
| **多层数据验证** | 6 级行情 Fallback 链（雪球 -> 腾讯 -> 新浪 -> 东财 -> TuShare Free -> TuShare Pro），失败可见、降级可追踪 |
| **条件增强分析** | 按需触发板块联动补涨、产业链 BOM 拆解、美股→A股映射、社交热榜加权 |
...
|-- tdx_reader.py         # (v5.1) 通达信 .day / .blk 文件解析器
|-- tdx_scanner.py        # (v5.1) 通达信全市场扫描引擎（5537 只 A 股）
|-- schools.py            # (v5.0) 8 大投资流派配置 + 搜索关键词轮换池
|-- technical.py          # (v5.1) 技术指标计算模块
|-- supply_chain.py       # (v4.0) 产业链 BOM 拆解 + 三高筛选
|-- catchup_engine.py     # (v3.6) 板块联动补涨引擎
|-- tier_runner.py        # (v5.1) 流派感知 Tier 分类运行器
|-- gen_report_from_data.py  # (v5.1) HTML 报告生成器 + 流派筛选 UI
|-- gen_data.py / run_analysis.py / run_tiers.py  # 编排脚本
|
shared/
|-- template_base.css     # 报告 HTML 基础样式
|-- template_base.js      # 报告交互 JS（流派筛选 UI）
|
tests/
|-- __init__.py
...
| 模块 | 职责 | 关键导出 |
|------|------|----------|
| `skill.md` | Skill 入口，定义触发条件、执行步骤、输出规范 | 触发词列表、9 步执行流程、自检清单 |
| `config.py` | 环境变量读取、凭据管理、路径配置 | `TUSHARE_TOKEN`, `TDX_PATH`, `REPORT_DIR`, `PROVIDER_LABELS` |
...
| `data_sources.py` | 多源行情抓取、美股涨幅榜、社交热榜并行采集 | `get_realtime_quote_fallback()`, `get_intraday_quote()`, `tushare_pro_daily_batch()`, `fetch_hot_trends()` |
| `tdx_reader.py` | 通达信本地 .day / .blk 文件二进制解析 | `TDXReader`, `scan_all_codes()`, `read_block_file()` |
| `tdx_scanner.py` | 通达信全市场扫描 + 板块检测 | `TDXScanner`, 全市场快照、板块涨停统计 |
| `schools.py` | 8 大投资流派定义、搜索关键词轮换池、流派匹配 | `InvestmentSchool`, `get_search_terms()`, `match_school()` |
| `technical.py` | MA/MACD/RSI/KDJ 等技术指标计算 | 基于 TDX 本地数据的指标批量计算 |
| `supply_chain.py` | 产业链上下游 BOM 拆解、三高评分 | `compute_three_high_bonus()`, `decompose_supply_chain()` |
| `catchup_engine.py` | 板块联动补涨机会检测 | 基于 TDX 概念板块数据的补涨扫描 |
| `gen_report_from_data.py` | 深色交易台风格 HTML 日报 + 流派筛选 UI | 命令行生成 HTML，带交互式流派切换 |
| `compute_tiers.py` | 独立可执行的 Tier 批量计算脚本 | 命令行运行，输出 JSON 格式评分结果 |
| `tier_runner.py` | 流派感知 Tier 分类运行器 | 按流派分组执行 Tier 评分 |
...
Step 0: 解析用户意图
   |
   v
Step 1: TDX 本地全市场扫描 (v5.1, 5537 只 A 股, 0.6s)
   |
   v
Step 2: 新闻检索 (WebSearch 多引擎搜索)
   |
   v
Step 3: 美股涨幅榜→A 股映射 (v3.7)
   |
   v
Step 4: 产业链 BOM 拆解 + 三高筛选 (v4.0)
   |
   v
Step 5: 股票识别与标准化 (代码 + 名称)
   |
   v
Step 6: 实时行情校验 (Python data_sources 模块)
   |      Fallback 链: 雪球 -> 腾讯 -> 新浪 -> 东方财富 -> TuShare Free -> TuShare Pro Daily
   |
   v
Step 7: 8 大投资流派匹配 + Tier 分配引擎 (v5.0)
   |      7a. 投资流派分类 (价值/成长/短线/技术/量化/事件驱动/宏观/投机)
   |      7b. 流派感知 Tier 评分 (6 因子加权公式)
   |      7c. 板块集中度检查 + 集中度降权
   |      7d. 按 tier_score 降序排序输出
   |
   v
Step 8: HTML 报告生成 + 流派筛选 UI + 自动打开 [强制门禁]
         - 深色交易台风格 (底色 #0a0e14, 卡片 #181d27)
         - 三级分层表格 (T1/T2/T3) + 流派切换筛选器
...
v5.1 引入通达信 (TDX) 本地数据读取能力，直接从本地 `vipdoc` 目录读取全市场 5537 只 A 股的日线数据，**零网络延迟**，全量扫描仅需 0.6 秒。

### 1. 下载安装通达信

从通达信官网下载并安装即可获得完整历史日线数据：

| 客户端 | 下载地址 | 说明 |
|--------|---------|------|
| **通达信金融终端 (免费)**| [http://www.tdx.com.cn](http://www.tdx.com.cn) | 免费版，包含 A 股全量日线数据 |
| **券商定制版** | 各券商官网（如华泰涨乐、中信建投等） | 与免费版数据目录相同结构 |

**安装后首次启动**：打开通达信客户端，让其自动下载历史日线数据（通常在首次启动时自动完成）。客户端会持续更新每日 K 线数据，无需额外操作。
...
| 系统 | 默认路径 |
|------|---------|
| **Windows** | `C:\new_tdx` (免费版) 或 `C:\Program Files\通达信` |
| **券商版 Windows** | 各券商安装目录，如 `C:\zd_zsmt` (华泰涨乐)、`C:\htzqzyb` 等 |

数据目录结构：
```
new_tdx/
├── vipdoc/
│   ├── sh/lday/       # 沪市日线 .day 文件（如 sh600519.day）
│   └── sz/lday/       # 深市日线 .day 文件（如 sz000858.day）
└── T0002/
    └── blocknew/      # 概念板块 .blk 文件（如 半导体.blk, 人工智能.blk）
```
...
# Linux / macOS (如果有)
export TDX_PATH="/path/to/new_tdx"
```
...
# 验证 .day 文件读取
reader = TDXReader("C:\\new_tdx")
codes = reader.scan_all_codes()  # 列出所有股票代码 + 最新收盘价
print(f"发现 {len(codes)} 只股票")

# 验证全市场扫描
scanner = TDXScanner("C:\\new_tdx")
snapshots = scanner.scan_all_market()  # 全市场快照
print(f"扫描完成: {len(snapshots)} 只股票, 耗时约 0.6s")
```

### 5. 数据更新

通达信客户端每次启动时会自动更新日线数据。确保每周至少打开一次客户端刷新数据，以保证选股时效性。也可以使用通达信自带的数据下载功能手动更新：

1. 打开通达信客户端
2. 菜单 -> **系统** -> **盘后数据下载**
3. 勾选 **日线数据**，选择日期范围，点击下载

> **注意**：`tdx_reader.py` 不依赖通达信客户端运行，不需要打开客户端即可读取本地 `.day` 文件。但需要客户端定期更新数据文件。

---

## 8 大投资流派 (v5.0)

v5.0 引入多流派选股体系，不同流派的投资者会自动匹配不同的搜索关键词和分析侧重点。

| 流派 | 标签 | 风格 | 偏好催化类型 | 搜索关键词示例 |
|------|------|------|-------------|--------------|
| **价值投资** | `VALUE` | 长线 | 业绩超预期、低估值 | "业绩增长 净利润" "低市盈率" |
| **成长投资** | `GROWTH` | 中线 | 技术突破、行业景气 | "营收高增长" "市占率提升" |
| **短线交易** | `MOMENTUM` | 短线 | 概念热点、资金流入 | "资金流入" "放量突破" |
| **技术分析** | `TECHNICAL` | 波段 | 技术形态、量价配合 | "均线多头" "MACD金叉" |
| **量化交易** | `QUANT` | 系统化 | 多因子、统计套利 | "因子暴露" "波动率异常" |
| **事件驱动** | `EVENT_DRIVEN` | 事件 | 并购重组、政策利好 | "并购重组" "政策受益" |
| **宏观对冲** | `MACRO` | 宏观 | 汇率、利率、行业轮动 | "降息受益" "通胀受益" |
| **投机交易** | `SPECULATIVE` | 超短 | 涨停板、情绪博弈 | "连板股" "龙虎榜" |

### 流派感知 Tier 引擎

按下式计算 tier 分数后，各流派自动施加流派专属后处理（技术派叠加技术指标、量化派叠加因子验证等）：
...
HTML 报告中可通过流派切换 UI 按流派筛选查看结果。

---

## 安装方式

快速安装：直接把仓库地址，丢给你的agent助手，让它帮你安装。

### 脚本安装

**Linux / macOS:**
```bash
curl -fsSL https://raw.githubusercontent.com/AXBIAO/news-stock-selector/master/install.sh | bash
```

**Windows (PowerShell):**
```powershell
irm https://raw.githubusercontent.com/AXBIAO/news-stock-selector/master/install.ps1 | iex
```

### 手动安装

1. **Clone 仓库并链接到 Claude Code skills 目录：**
   ```bash
   git clone https://github.com/AXBIAO/news-stock-selector.git ~/.claude/skills/news-stock-selector
   ```
   或将仓库克隆到任意位置后创建软链接：
   ```bash
   # macOS / Linux
...
> **注意**：v5.1 优先使用通达信本地数据 (`TDX_PATH`)，全市场 5537 只 A 股 0.6s 扫描完成。实时行情走 Python `data_sources.py` 模块（雪球 / 腾讯 / 新浪 / 东财 / TuShare 多源 Fallback 链）作为补充。
...
|-- data_sources.py              # 多源行情 + 社交热榜 + 美股映射
|-- tdx_reader.py                # (v5.1) 通达信 .day / .blk 二进制解析器
|-- tdx_scanner.py               # (v5.1) 通达信全市场扫描引擎
|-- schools.py                   # (v5.0) 8 大投资流派 + 搜索关键词轮换池
|-- technical.py                 # (v5.1) 技术指标计算 (MA/MACD/RSI/KDJ)
|-- supply_chain.py              # (v4.0) 产业链 BOM 拆解 + 三高筛选
|-- catchup_engine.py            # (v3.6) 板块联动补涨引擎
|-- tier_runner.py               # (v5.1) 流派感知 Tier 分类运行器
|-- compute_tiers.py             # Tier 独立计算脚本
|-- gen_report_from_data.py      # (v5.1) HTML 报告 + 流派筛选 UI 生成器
|-- gen_data.py                  # 数据编排脚本
|-- gen_report.py                # 报告生成编排脚本
|-- run_analysis.py              # 分析编排脚本
|-- run_tiers.py                 # Tier 编排脚本
|-- _gen_fund_report.py          # 资金流报告生成器
|-- shared/
|   |-- template_base.css        # 报告 HTML 基础样式
|   |-- template_base.js         # 报告交互 JS（流派筛选 UI）
|-- task_plan.md                 # 开发任务计划
...
| 2 | 报告数据真实 | 所有数据来自 `data_sources.py` 实际输出或 TDX 本地数据，非模板占位符 |
| 3 | 浏览器已打开 | 已执行 `start` 命令 |
| 4 | 报告结构完整 | 概览卡 + 分层表格 + 流派筛选 UI + 预判卡 + 策略建议 + 免责声明 |
| 5 | Tier 引擎已执行 | 每只股票的 `tier_score`, `assigned_tier`, `strategy_tag`, `school` 已填充 |
| 6 | 板块集中度已检查 | `overheat_warnings` 已填充，超限板块已降权 |
| 7 | TDX 已配置 (v5.1) | `TDX_PATH` 已设置，`tdx_scanner` 扫描成功 |
| 8 | TuShare 已配置 | `TUSHARE_TOKEN` 和 `TUSHARE_HTTP_URL` 已设置或确认跳过 |
...
- 通达信本地数据 (TDX) 集成：`.day` / `.blk` 文件二进制解析器 (`tdx_reader.py`)
- 全市场扫描引擎 (`tdx_scanner.py`)：5537 只 A 股全量扫描仅需 0.6 秒
- 技术指标模块 (`technical.py`)：MA / MACD / RSI / KDJ 批量计算
- 板块联动补涨引擎 (`catchup_engine.py`)：基于 TDX 概念板块数据检测补涨机会
- 流派感知 Tier 分类运行器 (`tier_runner.py`)
- HTML 报告生成器 (`gen_report_from_data.py`) + 交互式流派筛选 UI
- 共享模板 (`shared/`)：`template_base.css` + `template_base.js`

### v5.0.0

**8 大投资流派体系**

- 8 大投资流派引擎 (`schools.py`)：价值、成长、短线、技术、量化、事件驱动、宏观、投机
- 流派感知搜索关键词轮换池：避免搜索重复化
- 流派感知 Tier 引擎：不同流派自动匹配不同催化剂权重
- HTML 报告流派筛选 UI：按投资风格切换视图

### v4.0.0

**产业链 BOM 拆解 + 三高筛选**

- 产业链上下游 BOM 拆解 (`supply_chain.py`)
- 三高筛选体系（高景气 / 高壁垒 / 高弹性）
- 三高评分因子并入 Tier 引擎（权重 20%）
